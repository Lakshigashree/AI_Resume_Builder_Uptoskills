...
const Template3 = () => {
  ...
  const sectionTitleStyle = {
    fontWeight: "700", fontSize: "1.3rem", letterSpacing: "1px",
    fontFamily: FONT_HEADER, color: PRIMARY_TEXT_COLOR, margin: 0,
    textTransform: "uppercase"
  };

  const editBoxStyle = editMode ? {
    border: "1px dashed #3b82f6", backgroundColor: "#f9fafb", padding: "15px", borderRadius: "8px", marginBottom: "2rem"
  } : { marginBottom: "2.5rem" };

  const inputStyle = { width: "100%", padding: "8px", border: "1px solid #d1d5db", borderRadius: "4px", marginBottom: "5px" };
  ...
  const sectionComponents = {
    summary: hasContent(localData, "summary", editMode) && (
      <div key="summary" style={editBoxStyle}>
        ...
        ) : <p style={{ lineHeight: "1.7", color: PRIMARY_TEXT_COLOR, textAlign: "justify" }}>{localData.summary}</p>}
      </div>
    ),
    experience: hasContent(localData, "experience", editMode) && (
      <div key="experience" style={editBoxStyle}>
        <div style={sectionTitleContainerStyle}>
          ...
          {editMode && <button onClick={() => addItem("experience", {title:"", companyName:"", date:"", description:""})} style={{background:ACCENT_COLOR, color:"white", border:"none", borderRadius:"4px", padding:"2px 8px"}}>+</button>}
        </div>
        {safeArray(localData.experience).map((exp, i) => (
          <div key={i} style={{ marginBottom: "1.5rem" }}>
            {editMode ? (
              ...
              <div style={{ borderLeft: `2px solid ${SECTION_HEADER_BG}`, paddingLeft: "1rem" }}>
                <h4 style={{ margin: 0, fontSize: "1.1rem", fontWeight: "700", color: ACCENT_COLOR, fontFamily: FONT_HEADER }}>{renderSafe(exp.title)}</h4>
                <p style={{ margin: "0.2rem 0", fontSize: "0.95rem", fontWeight: "600" }}>{renderSafe(exp.companyName)} | {exp.date}</p>
                <p style={{ fontSize: "0.9rem", lineHeight: "1.6" }}>{renderSafe(exp.description)}</p>
              </div>
            )}
          </div>
        ))}
    ...
    education: hasContent(localData, "education", editMode) && (
      <div key="education" style={editBoxStyle}>
        <div style={sectionTitleContainerStyle}>
          ...
          {editMode && <button onClick={() => addItem("education", {degree:"", institution:"", duration:""})} style={{background:ACCENT_COLOR, color:"white", border:"none", borderRadius:"4px", padding:"2px 8px"}}>+</button>}
        </div>
        {safeArray(localData.education).map((edu, i) => (
          <div key={i} style={{ marginBottom: "1rem" }}>
            {editMode ? (
              ...
              <div style={{ borderLeft: `2px solid ${SECTION_HEADER_BG}`, paddingLeft: "1rem" }}>
                <h4 style={{ margin: 0, fontWeight: "700", color: ACCENT_COLOR }}>{renderSafe(edu.degree)}</h4>
                <p style={{ margin: 0 }}>{renderSafe(edu.institution)} | {edu.duration}</p>
              </div>
            )}
          </div>
        ))}
    ...
    skills: hasContent(localData, "skills", editMode) && (
      <div key="skills" style={editBoxStyle}>
        <div style={sectionTitleContainerStyle}>
          ...
          {editMode && <button onClick={() => addItem("skills", "")} style={{background:ACCENT_COLOR, color:"white", border:"none", borderRadius:"4px", padding:"2px 8px"}}>+</button>}
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
          {safeArray(localData.skills).map((s, i) => (
            <div key={i} style={{ backgroundColor: SECTION_HEADER_BG, padding: "5px 12px", borderRadius: "4px", fontSize: "0.9rem" }}>
    ...
    projects: hasContent(localData, "projects", editMode) && (
      <div key="projects" style={editBoxStyle}>
        <div style={sectionTitleContainerStyle}>
          ...
          {editMode && <button onClick={() => addItem("projects", {name:"", description:""})} style={{background:ACCENT_COLOR, color:"white", border:"none", borderRadius:"4px", padding:"2px 8px"}}>+</button>}
        </div>
        {safeArray(localData.projects).map((p, i) => (
          <div key={i} style={{ marginBottom: "1rem" }}>
            {editMode ? (
              ...
              <div style={{ borderLeft: `2px solid ${SECTION_HEADER_BG}`, paddingLeft: "1rem" }}>
                <h4 style={{ margin: 0, fontWeight: "700", color: ACCENT_COLOR }}>{renderSafe(p.name)}</h4>
                <p style={{ fontSize: "0.9rem" }}>{renderSafe(p.description)}</p>
              </div>
            )}
          </div>
        ))}
    ...
    certifications: hasContent(localData, "certifications", editMode) && (
        <div key="certifications" style={editBoxStyle}>
          <div style={sectionTitleContainerStyle}>
            ...
            {editMode && <button onClick={() => addItem("certifications", {title:""})} style={{background:ACCENT_COLOR, color:"white", border:"none", borderRadius:"4px", padding:"2px 8px"}}>+</button>}
          </div>
    ...
    achievements: hasContent(localData, "achievements", editMode) && (
        <div key="achievements" style={editBoxStyle}>
          <div style={sectionTitleContainerStyle}>
            ...
            {editMode && <button onClick={() => addItem("achievements", "")} style={{background:ACCENT_COLOR, color:"white", border:"none", borderRadius:"4px", padding:"2px 8px"}}>+</button>}
          </div>
    ...
    languages: hasContent(localData, "languages", editMode) && (
      <div key="languages" style={editBoxStyle}>
        <div style={sectionTitleContainerStyle}><h3 style={sectionTitleStyle}>Languages</h3></div>
    ...
    interests: hasContent(localData, "interests", editMode) && (
      <div key="interests" style={editBoxStyle}>
        <div style={sectionTitleContainerStyle}><h3 style={sectionTitleStyle}>Interests</h3></div>
    ...
    <div style={{ minHeight: "100vh", backgroundColor: LIGHT_BACKGROUND }}>
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{ flexGrow: 1, padding: "2.5rem", display: "flex", justifyContent: "center" }}>
          <div ref={resumeRef} style={{ backgroundColor: "#ffffff", width: "794px", minHeight: "1123px", padding: "4rem", borderRadius: "8px", boxShadow: "0px 10px 30px rgba(0,0,0,0.1)", fontFamily: FONT_BODY, color: PRIMARY_TEXT_COLOR }}>
            
            {/* HEADER */}
            <div style={{ marginBottom: "3rem", textAlign: "center", borderBottom: `3px solid ${ACCENT_COLOR}`, paddingBottom: "1.5rem" }}>
              {editMode ? (
                <div style={{display:"grid", gap:"10px"}}>
                  <input style={{ fontSize: "2.5rem", fontWeight: "900", textAlign: "center" }} value={localData.name} onChange={(e) => handleFieldChange("name", e.target.value)} placeholder="Full Name" />
                  <input style={{ fontSize: "1.2rem", textAlign: "center", color: ACCENT_COLOR }} value={localData.role} onChange={(e) => handleFieldChange("role", e.target.value)} placeholder="Role" />
                </div>
              ) : (
                <>
                  <h1 style={{ fontSize: "3rem", fontWeight: "900", textTransform: "uppercase", letterSpacing: "2px", margin: 0, fontFamily: FONT_HEADER }}>{localData.name || "Your Name"}</h1>
                  <h2 style={{ fontSize: "1.3rem", color: ACCENT_COLOR, fontWeight: "600", margin: "5px 0" }}>{localData.role || "Professional Role"}</h2>
                </>
              )}

              {/* REDIRECTING LINKS */}
              <div style={{ marginTop: "1.5rem", display: "flex", justifyContent: "center", flexWrap: "wrap", gap: "1.5rem", fontSize: "0.9rem" }}>
                {editMode ? (
                  ...
                  <>
                    {localData.phone && <a href={`tel:${localData.phone}`} style={{ textDecoration: "none", color: "inherit" }}>📞 {localData.phone}</a>}
                    {localData.email && <a href={`mailto:${localData.email}`} style={{ textDecoration: "none", color: "inherit" }}>✉️ {localData.email}</a>}
                    {localData.linkedin && <a href={normalizeUrl(localData.linkedin)} target="_blank" rel="noreferrer" style={{ textDecoration: "none", color: ACCENT_COLOR, fontWeight: "bold" }}>LinkedIn</a>}
                    {localData.github && <a href={normalizeUrl(localData.github)} target="_blank" rel="noreferrer" style={{ textDecoration: "none", color: ACCENT_COLOR, fontWeight: "bold" }}>GitHub</a>}
                    {localData.portfolio && <a href={normalizeUrl(localData.portfolio)} target="_blank" rel="noreferrer" style={{ textDecoration: "none", color: ACCENT_COLOR, fontWeight: "bold" }}>Portfolio</a>}
                  </>
                )}
              </div>
            </div>
            ...
            <div data-html2canvas-ignore="true" style={{ textAlign: "center", marginTop: "3rem" }}>
              {editMode ? (
                <div style={{display:"flex", justifyContent:"center", gap:"15px"}}>
                  <button onClick={handleSave} style={{ background: ACCENT_COLOR, color: "white", padding: "10px 25px", borderRadius: "6px", border: "none", cursor: "pointer", fontWeight: "bold" }}>Save Changes</button>
                  <button onClick={handleCancel} style={{ background: "#6c757d", color: "white", padding: "10px 25px", borderRadius: "6px", border: "none", cursor: "pointer", fontWeight: "bold" }}>Cancel</button>
                </div>
              ) : (
                <button onClick={() => setEditMode(true)} style={{ background: ACCENT_COLOR, color: "white", padding: "12px 40px", borderRadius: "6px", border: "none", cursor: "pointer", fontWeight: "bold" }}>Edit Resume</button>
              )}
            </div>
          </div>
        </div>
